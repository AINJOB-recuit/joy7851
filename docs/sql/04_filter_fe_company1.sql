-- =============================================================================
-- 목적: 기업1 FE 공고 지원자 중 학력·경력·스킬 조건을 만족하는 후보 추출
-- 전제/가정:
--   - "합격자" = 조건 충족 지원자
--   - stage·status 는 분류용으로 SELECT, 필요 시 AND 로 필터
-- 인덱스 전제:
...
WITH params AS (
    SELECT
        ...
        (SELECT id FROM position_type WHERE code = 'FRONTEND') AS frontend_position_id,
        (SELECT id FROM skill WHERE skill_key = 'REACT') AS react_skill_id,
        (SELECT id FROM skill WHERE skill_key = 'NEXTJS') AS nextjs_skill_id,
        (SELECT id FROM skill WHERE skill_key = 'TYPESCRIPT') AS typescript_skill_id
),
required_skills AS (
    SELECT react_skill_id AS skill_id FROM params
    UNION ALL
    SELECT nextjs_skill_id FROM params
    UNION ALL
    SELECT typescript_skill_id FROM params
),
candidate_applications AS (
    ...
    WHERE a.company_id = p.company_id
      ...
      AND jp.position_type_id = p.frontend_position_id
),
...
frontend_career AS (
    SELECT c.applicant_id, SUM(c.years) AS frontend_years
    FROM candidate_applicants ca
    JOIN career c ON c.applicant_id = ca.applicant_id
    CROSS JOIN params p
    WHERE c.position_type_id = p.frontend_position_id
    GROUP BY c.applicant_id
    HAVING SUM(c.years) >= 3
),
qualified_skills AS (
...
JOIN frontend_career fc ON fc.applicant_id = ca.applicant_id
...
-- (참고) 면접+보류: AND st.sort_order = 2 AND ss.code = 'PENDING'
